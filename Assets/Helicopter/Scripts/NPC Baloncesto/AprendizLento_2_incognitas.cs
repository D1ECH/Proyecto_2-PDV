...
public class AprendizLento_2_incognitas : MonoBehaviour
{
    ...
    IEnumerator Entrenamiento()
    {
        ...
        if (casosEntrenamiento.numInstances() < 10)
        {
            ...
            for (float Fx = 16; valorMinimoFx <= valorMaximoFx; Fx = Fx + paso)                      //Bucle de planificación de la fuerza FX durante el entrenamiento
            {
                for (float Fy = valorMinimoFy; Fy <= valorMaximoFy; Fy = Fy + paso)                    //Bucle de planificación de la fuerza FY durante el entrenamiento
                {
                    // InstanciaPelota = Instantiate(pelota, npcTransform) as GameObject;
                    InstanciaPelota = Instantiate(pelota, this.transform.position, this.transform.rotation);
                    print("INSTANCIA PELOTA");
                    ...
                    yield return new WaitUntil(() => rb.transform.position.y <= 10f || Time.time - startTime >= 10f);       //... y espera a que la pelota llegue al suelo
                    print("WAIT UNTIL");
